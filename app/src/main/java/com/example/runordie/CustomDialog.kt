...
class CustomDialog : Dialog { //继承Dialog类
    constructor(context: Context):super(context){}  //调用父类的构造方法
    constructor(context:Context, theme:Int):super(context,theme){}

    class Builder(context: Context){ //Bulider模式构建
        ...
        private fun create() {
            if (message != null) {      //设置提示内容
                ...
                (layout.findViewById<View>(R.id.content) as LinearLayout).removeAllViews() //首先删除所有布局
                (layout.findViewById<View>(R.id.content) as LinearLayout) //线性的页面，后一个参数是和父页面一样大小
